...
# Cap on points fed into the significant-places DBSCAN. A week-long, low-
# mobility trace can have tens of thousands of fixes almost all mutually
# within HOME_RADIUS_M of each other, which makes neighbor-list construction
# blow up in memory regardless of ball_tree vs brute force. A systematic
# (every-Nth-row) subsample preserves the time-ordered spatial distribution
# while keeping clustering tractable.
...
def daily_gps_features(gps, lat_col, lon_col, home):
    ...
    for date, day in gps.groupby("date"):
        ...
        # datetime64. Casting through int64 nanoseconds keeps dt as a real
...
def location_entropy(gps, lat_col, lon_col):
    ...
    if len(pts) > ENTROPY_MAX_POINTS:
        # Systematic (every-Nth-row) subsample rather than random, so the
        # result is reproducible and keeps time-ordered spatial coverage
        # rather than clumping toward whichever rows a random draw picks.
        stride = int(np.ceil(len(pts) / ENTROPY_MAX_POINTS))
        pts = pts.iloc[::stride]
    coords = np.radians(pts.to_numpy())
    eps = HOME_RADIUS_M / 6_371_000.0
    # algorithm="ball_tree" avoids building a full O(n^2) pairwise distance
    # matrix, which otherwise exhausts memory on dense full-trace inputs
    # (e.g. ~30k fixes needs ~7.4 GB as a brute-force distance matrix). Even
    # with ball_tree, a low-mobility week can still blow up memory because
    # nearly every point falls within eps of every other point; the point
    # cap above is what actually keeps this bounded.
    labels = DBSCAN(eps=eps, min_samples=5, metric="haversine",
                     algorithm="ball_tree").fit_predict(coords)
    valid = labels[labels >= 0]
    if valid.size == 0:
...
def extract_home_survey(participant_dir):
    """
    Pull the self-reported 'hours spent at home' answers.

    The public sample study asked one question: how many hours the participant
    thought they spent at home. This is the only self-report variable in the
    dataset and it is the ground truth for validation.
    """
    for stream in ("survey_answers", "survey_timings"):
        df = load_stream(participant_dir, stream)
        if df.empty:
            continue
        ...
        out = df[["t_local", "date", ans_col]].copy()
        out["reported_home_h"] = pd.to_numeric(out[ans_col], errors="coerce")
        out = out.dropna(subset=["reported_home_h"])
        # survey_timings repeats rows per event; keep one answer per prompt.
        out = out.drop_duplicates(subset=["date", "reported_home_h"])
        if not out.empty:
            return out[["t_local", "date", "reported_home_h"]]
